function successors=explore(x_cell,y_cell,h,x_target,y_target,in_valid,max_x,max_y)
    successors=[];
    successor_count=1;
    c2=size(in_valid,1);
    for k= 1:-1:-1
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=max_x) && (s_y >0 && s_y <=max_y)) % successor within the matrix
                    ...
                    if (flag == 1)
                        ...
                        successors(successor_count,3) = h+distance2(x_cell,y_cell,s_x,s_y);%h
                        successors(successor_count,4) = distance2(x_target,y_target,s_x,s_y);%g
